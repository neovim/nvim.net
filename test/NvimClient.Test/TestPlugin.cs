...
internal class TestPlugin {
    ...
    public TestPlugin(NvimAPI nvim) {
        _nvim = nvim;
    }
    ...
    public long CountLines(NvimRange range) {
        long lineCount = range.LastLine - range.FirstLine + 1;
        _ = _nvim.OutWrite($"Function {nameof(CountLines)} called with {lineCount} lines in range");
        CountLinesReturn = lineCount;
        return lineCount;
    }

    [NvimCommand(NArgs = "*")]
    public static void TestCommand1(string[] args) {
        Command1Args = args;
    }

    [NvimCommand(NArgs = "?")]
    public static void TestCommand2(string optionalArg) {
    ...
    public void OnBufEnter([NvimEval("expand('<afile>')")] string filename,
      [NvimEval("&shiftwidth")] long shiftWidth) {
        string indent = new(' ', (int)shiftWidth);
        _ = _nvim.SetCurrentLine(indent + $"{nameof(OnBufEnter)} called with {filename}");
        AutocmdCalled = true;
    }
}